...
* Coder puis commit le code sur votre branche. `git add --all && git commit -m "Mon message"`

* Envoyer le code sur le serveur github `git push`

* Enfin, sur l'interface web de github ouvrir une `pull request`.

À ce moment votre code sera étudié par les autres développeurs et sera accepté ou rejeté. Des retours et des conseils vous serons fait s'il y a des modifications à faire.
